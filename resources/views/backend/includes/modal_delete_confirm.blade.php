    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                @endif
                @if($type == 'customers')
                    <form action="{{ route('customer.forceDelete', ['id' => $item->id]) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger">Delete</button>
                    </form>
                @endif
                @if($type == 'comments')
                    <form action="{{ route('comments.forceDelete', ['id' => $item->id]) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger">Delete</button>
                    </form>
                ...
                @endif
                @if($type == 'introduces')
                    <form action="{{ route('introduces.forceDelete', ['id' => $item->id]) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger">Delete</button>
                    </form>
