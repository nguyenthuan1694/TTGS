...
@section('content')
    ...
    <div class="container-fluid px-xxl-65 px-xl-20">
        ...
        <div class="hk-pg-header">
            <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>List of news</h4>
            <div class="d-flex">
                <a href="{{ route('news.create') }}" class="btn btn-info small">Create</a>
            </div>
        </div>
        <!-- /Title -->
                            ...
                            <div class="table-wrap table-responsive">
                                <table class="table table-striped table-bordered">
                                    ...
                                    @foreach($news as $index => $items)
                                        <tr>
                                            <td>{{ $items->id }}</td>
                                            <td><img src="{{ asset($items->thumbnail) }}" class="img-fluid img-thumbnail" height="100" width="100" alt="img"></td>
                                            <td>{{ $items->title }}</td>
                                            <td>{{ $items->created_at->diffForHumans() }}</td>
                                            <td>{{ $items->updated_at->diffForHumans() }}</td>
                                            <td>
                                                <a href="{{ route('news.edit', ['news' => $items->id]) }}" class="mr-25" data-toggle="tooltip" data-placement="top" title="Edit"> <i class="icon-pencil"></i> </a>
