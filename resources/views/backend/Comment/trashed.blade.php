...
@section('content')
    ...
    <div class="container-fluid px-xxl-65 px-xl-20">
        ...
        <div class="hk-pg-header">
            <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>List of trashed Comment</h4>
        </div>
        <!-- /Title -->
                                    ...
                                    @foreach($comments as $index => $comment)
                                        ...
                                        <td>{{ $comment->name }}</td>
                                            ...
                                            <td>
                                                ...
                                                <form name="restore_{{ $index }}" action="{{ route('comments.restore', ['id' => $comment->id]) }}" method="post">
                                                    @csrf
                                                    @method('PUT')
                                                </form>
                                                @include('backend.includes.modal_delete_confirm', ['index' => $index, 'item' => $comment, 'type' => 'comments'])
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if(!count($comments))
                    ...
                    <div class="row">
                        <div class="col-7">
                            <div class="float-left">
                                {!! $comments->total() !!} trashed comments
                            </div>
                        </div>
