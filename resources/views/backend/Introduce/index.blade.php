...
@section('content')
    ...
    <div class="container-fluid px-xxl-65 px-xl-20">
        ...
        <div class="hk-pg-header">
            <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>List of Introduce</h4>
            <div class="d-flex">
                <a href="{{ route('introduces.create') }}" class="btn btn-info small">Creates</a>
            </div>
        </div>
        <!-- /Title -->
                        ...
                        <div class="col-sm">
                            <div class="table-wrap table-responsive">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                    @foreach($introduces as $index => $items)
                                        <tr>
                                            <td>{{ $items->id }}</td>
                                            <td>{{ $items->title }}</td>
                                            ...
                                            <td>
                                                <a href="{{ route('introduces.edit', ['introduce' => $items->id]) }}" class="mr-25" data-toggle="tooltip" data-placement="top" title="Edit"> <i class="icon-pencil"></i> </a>
                                                ...
                                                <form name="trash_{{ $index }}" action="{{ route('introduces.destroy', ['introduce' => $items]) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
