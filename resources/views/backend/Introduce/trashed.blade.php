...
@section('content')
    ...
    <div class="container-fluid px-xxl-65 px-xl-20">
        ...
        <div class="hk-pg-header">
            <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>List of trashed Introduce</h4>
        </div>
        <!-- /Title -->
                        ...
                        <div class="col-sm">
                            <div class="table-wrap table-responsive">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                    @foreach($introduce as $index => $items)
                                        <tr>
                                            <td>{{ $items->id }}</td>
                                            <td>{{ $items->title }}</td>
                                            ...
                                            <td>
                                                ...
                                                <form name="restore_{{ $index }}" action="{{ route('introduces.restore', ['id' => $items->id]) }}" method="post">
                                                    @csrf
                                                    @method('PUT')
                                                </form>
                                                @include('backend.includes.modal_delete_confirm', ['index' => $index, 'item' => $items, 'type' => 'introduces'])
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if(!count($introduce))
                    ...
                    <div class="row">
                        <div class="col-7">
                            <div class="float-left">
                                {!! $introduce->total() !!} trashed introduce
                            </div>
                        </div>
